...
// Load Events from API (replaces static version)
async function loadEvents() {
    const grid = document.getElementById('eventsGrid');
    if (!grid) return;

    const country  = (document.getElementById('eventCountry')  || {}).value  || 'all';
    const category = (document.getElementById('eventCategory') || {}).value || 'all';

    let url = `${API_BASE_URL}/events.php`;
    const params = [];
    if (country  !== 'all') params.push(`country=${encodeURIComponent(country)}`);
    if (category !== 'all') params.push(`category=${encodeURIComponent(category)}`);
    if (params.length) url += '?' + params.join('&');

    grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:#888">Loading events...</div>';
    ...
    try {
        ...
        if (data.success && data.data.length > 0) {
            grid.innerHTML = data.data.map(ev => {
                ...
                return `
                <div class="event-card" data-country="${ev.country}" data-category="${ev.category}">
                    ...
                    <div class="event-info">
                        ...
                        <button class="btn-register" onclick="viewEventDetails(${ev.id})">View Details &amp; Register</button>
                    </div>
                </div>`;
            }).join('');
        } else {
            grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:#888"><i class="fas fa-calendar-times" style="font-size:2rem;margin-bottom:12px;display:block"></i>No upcoming events found. Check back soon!</div>';
        }
    } catch (err) {
        console.error('loadEvents error:', err);
        grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:#e55">Failed to load events. Please refresh.</div>';
    }
}

// filterEvents now re-calls loadEvents with current filter values
